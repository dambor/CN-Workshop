...
. By default Spring Boot will use Spring Security to protect these management endpoints (which is a good thing!) We’ll set a simple password to make demonstration a bit easier and simpler.  Additionally, we need to publish these endpoints via REST.
+
...
----
+
Try out the following endpoints. Remember that you set the password to _p1v0tal_ and the default username is _user_. The output is omitted here because it can be quite large:
